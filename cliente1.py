...
logCli1 = open('logCli1.txt','w') # Abre arquivo escrita do logServer1
...
while (port < 8889): #Loop para iterar a porta enviando a msg para os 3 servidores
	...
	logCli1.write(datelog + '---->' + 'CONECTADO AO SERVIDOR : ' + remote_ip + ':' + str(port)+ '\n')
	...
	reply = s.recv(1024)
	if(reply == 'ok'):
		logCli1.write(datelog + '---->' + 'RECEBIDO OK DO SERVIDOR: '+ remote_ip + ':' + str(port)+ '\n')
		qnt_ok = qnt_ok + 1
		print qnt_ok
	print reply
	port = port + 1
...
if(qnt_ok == 3):
	...
	port = 8886;
	logCli1.write(datelog + '---->' + 'PREPARANDO PARA ALTERAR NO SERVIDOR: '+ remote_ip + ':' + str(port)+ '\n')
	while (port < 8889): #Loop para iterar a porta enviando a msg para os 3 servidores
		try:
		    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		except socket.error:
		...
		try :
		    ...
		    s.sendall(message)
		    logCli1.write(datelog + '---->' + 'ALTERANDO VARIAVEL NO SERVIDOR : '+ remote_ip + ':' + str(port)+ '\n')
		    time.sleep(1) # Variavel usada pra dar um tempo de 1 segundo antes de enviar para o proximo servidor	
		except socket.error:
		    #Caso falhe
		    print 'Falha no Envio'
...
######################## CASO RECEBA UM ABORT DE ALGUN DOS 3 ENVIA UM ABORT DA OPERACAO PARA TODOS ############################################
	...
	port = 8886;
	logCli1.write(datelog + '---->' + 'NAO FOI POSSIVEL ALTERAR ABORTANDO NO SERVIDOR : '+ remote_ip + ':' + str(port)+ '\n')
